...
void Measure::ExecuteBang(LPCWSTR args)
{
    ...
    wstring sargs = args;
            ...
            /*
    switch (SetValueA(CommandMap, sargs, COMMAND::DEFAULT))
    {
        case 1:
            break;
        ...
        case COMMAND::BLUR_DISABLE:
            ...
            break;
        case COMMAND::BACKDROP::TOGGLE:
            if (m_Backdrop[1] == BACKDROP::DARK || m_Backdrop[1] == BACKDROP::LIGHT)
            {
                m_Backdrop[1] = m_Backdrop[1] == BACKDROP::DARK ? BACKDROP::LIGHT : BACKDROP::DARK;
            }
            if (m_Backdrop[1] == BACKDROP::DARK2 || m_Backdrop[1] == BACKDROP::LIGHT2)
            {
                m_Backdrop[1] = m_Backdrop[1] == BACKDROP::DARK2 ? BACKDROP::LIGHT2 : BACKDROP::DARK2;
            }
            if (m_Backdrop[1] == BACKDROP::DARK3 || m_Backdrop[1] == BACKDROP::LIGHT3)
            {
                m_Backdrop[1] = m_Backdrop[1] == BACKDROP::DARK3 ? BACKDROP::LIGHT3 : BACKDROP::DARK3;
            }
            if (m_Backdrop[1] == BACKDROP::DARK4 || m_Backdrop[1] == BACKDROP::LIGHT4)
            {
                m_Backdrop[1] = m_Backdrop[1] == BACKDROP::DARK4 ? BACKDROP::LIGHT4 : BACKDROP::DARK4;
            }
            if (m_Backdrop[1] == BACKDROP::DARK5 || m_Backdrop[1] == BACKDROP::LIGHT5)
            {
                m_Backdrop[1] = m_Backdrop[1] == BACKDROP::DARK5 ? BACKDROP::LIGHT5 : BACKDROP::DARK2;
            }
            if (m_Backdrop[1] == BACKDROP::DARK_BASE || m_Backdrop[1] == BACKDROP::LIGHT_BASE)
            {
                m_Backdrop[1] = m_Backdrop[1] == BACKDROP::DARK_BASE ? BACKDROP::LIGHT_BASE : BACKDROP::DARK_BASE;
            }
            API::WindowStyler::SetAccent(m_Hwnd, m_Accent[1], m_Effect[1], m_Shadow[1], m_Backdrop[1]);
            break;
        case COMMAND::BACKDROP::LIGHTMODE:
            if (m_Backdrop[1] == BACKDROP::DARK)
            {
                m_Backdrop[1] = BACKDROP::LIGHT;
            }
            if (m_Backdrop[1] == BACKDROP::DARK2)
            {
                m_Backdrop[1] = BACKDROP::LIGHT2;
            }
            if (m_Backdrop[1] == BACKDROP::DARK3)
            {
                m_Backdrop[1] = BACKDROP::LIGHT3;
            }
            if (m_Backdrop[1] == BACKDROP::DARK4)
            {
                m_Backdrop[1] = BACKDROP::LIGHT4;
            }
            if (m_Backdrop[1] == BACKDROP::DARK5)
            {
                m_Backdrop[1] = BACKDROP::LIGHT5;
            }
            if (m_Backdrop[1] == BACKDROP::DARK_BASE)
            {
                m_Backdrop[1] = BACKDROP::LIGHT_BASE;
            }
            API::WindowStyler::SetAccent(m_Hwnd, m_Accent[1], m_Effect[1], m_Shadow[1], m_Backdrop[1]);
            break;
        case COMMAND::BACKDROP::DARKMODE:
            if (m_Backdrop[1] == BACKDROP::LIGHT)
            {
                m_Backdrop[1] = BACKDROP::DARK;
            }
            if (m_Backdrop[1] == BACKDROP::LIGHT2)
            {
                m_Backdrop[1] = BACKDROP::DARK2;
            }
            if (m_Backdrop[1] == BACKDROP::LIGHT3)
            {
                m_Backdrop[1] = BACKDROP::DARK3;
            }
            if (m_Backdrop[1] == BACKDROP::LIGHT4)
            {
                m_Backdrop[1] = BACKDROP::DARK4;
            }
            if (m_Backdrop[1] == BACKDROP::LIGHT5)
            {
                m_Backdrop[1] = BACKDROP::DARK5;
            }
            if (m_Backdrop[1] == BACKDROP::LIGHT_BASE)
            {
                m_Backdrop[1] = BACKDROP::DARK_BASE;
            }
            API::WindowStyler::SetAccent(m_Hwnd, m_Accent[1], m_Effect[1], m_Shadow[1], m_Backdrop[1]);
            break;
        case COMMAND::CORNER::TOGGLE:
            if (System::VersionState::IsWin11())
            {
                if (m_Accent[1] != ACCENT::DEFAULT || m_Mica[1] != MICA::DEFAULT)
                {
        ...
        case COMMAND::CORNER::DISABLE:
            ...
            break;
        case COMMAND::STROKECOLOR::TOGGLE:
            if (System::VersionState::IsWin11())
            {
                if (m_Accent[1] != ACCENT::DEFAULT || m_Mica[1] != MICA::DEFAULT)
                {
                    if (m_StrokeColor[1] == STROKECOLOR::HIDDEN)
                    {
                        m_StrokeColor[1] = m_AdvancedOptions_32_t[2][1];
                        m_AdvancedOptions_32_t[2][1] = STROKECOLOR::HIDDEN;
                    }
                    else
                    {
                        m_AdvancedOptions_32_t[2][1] = m_StrokeColor[1];
                        m_StrokeColor[1] = STROKECOLOR::HIDDEN;
                    }
                    API::WindowStyler::SetStrokeColor(m_Hwnd, m_StrokeColor[1]);
                }
            }
            break;
        case COMMAND::STROKECOLOR::ENABLE:
            if (System::VersionState::IsWin11())
            {
                if (m_Accent[1] != ACCENT::DEFAULT || m_Mica[1] != MICA::DEFAULT)
                {
            ...
            break;
        case COMMAND::STROKECOLOR::DISABLE:
            if (System::VersionState::IsWin11())
            {
                if (m_Accent[1] != ACCENT::DEFAULT || m_Mica[1] != MICA::DEFAULT)
                {
            ...
            break;
        case COMMAND::DARK_MODE_TOGGLE:
            if (System::VersionState::IsWin11())
            {
                m_IsDarkMode[1] = !m_IsDarkMode[1];
                API::WindowStyler::SetDarkMode(m_Hwnd, m_IsDarkMode[1]);
            }
            break;
        case COMMAND::DARK_MODE_ENABLE:
            if (System::VersionState::IsWin11())
            {
                m_IsDarkMode[1] = FALSE;
                API::WindowStyler::SetDarkMode(m_Hwnd, m_IsDarkMode[1]);
            }
            break;
        case COMMAND::DARK_MODE_DISABLE:
            if (System::VersionState::IsWin11())
            {
                m_IsDarkMode[1] = TRUE;
                API::WindowStyler::SetDarkMode(m_Hwnd, m_IsDarkMode[1]);
            ...
            break;
        case COMMAND::MICA::FOCUS_TOOGLE:
            if (System::VersionState::IsMica())
            {
                m_IsMicaOnFocus[1] = !m_IsMicaOnFocus[1];
                API::WindowStyler::SetMica(m_Hwnd, m_Mica[1], m_IsMicaOnFocus[1]);
            }
            break;
        case COMMAND::MICA::FOCUS_ENABLE:
            if (System::VersionState::IsMica())
            {
                m_IsMicaOnFocus[1] = true;
                API::WindowStyler::SetMica(m_Hwnd, m_Mica[1], m_IsMicaOnFocus[1]);
            }
            break;
        case COMMAND::MICA::FOCUS_DISABLE:
            if (System::VersionState::IsMica())
            {
                m_IsMicaOnFocus[1] = false;
                API::WindowStyler::SetMica(m_Hwnd, m_Mica[1], m_IsMicaOnFocus[1]);
            }
            break;
        default:
